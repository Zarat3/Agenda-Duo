...
import { Calendar, Users, PlusCircle, Settings, X, LogOut, ShieldCheck, Bell, BellOff, Smartphone, Home, Building2, Clock, Download, UserCircle, Stethoscope, KeyRound, Mail, CheckCircle, AlertCircle, Palette, MessageSquarePlus } from 'lucide-react';
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map((item) => (
            <NavLink
              ...
              style={({ isActive }) => isActive ? { backgroundColor: 'var(--duo-accent, #800000)' } : {}}
            >
              <item.icon size={18} />
              {item.label}
            </NavLink>
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {CORES_PRESET.map(cor => (
                          <button key={cor} type="button" onClick={() => setCor(key, cor)}
                            className={`w-7 h-7 rounded-full transition-all border-2 ${cores[key] === cor ? 'border-[#1A1A1A] scale-110' : 'border-transparent'}`}
                        ...
                        <input type="color" value={cores[key] || '#800000'}
                          ...
                          className="w-7 h-7 rounded-full border border-[#DADADA] cursor-pointer p-0 overflow-hidden"
